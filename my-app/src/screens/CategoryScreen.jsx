...
const translatedValueCache = new Map();

function buildTranslateCacheKey(targetLanguage, text) {
...
async function translateValue(value, targetLanguage) {
  ...
  if (typeof value === "string") {
    ...
    const cacheKey = buildTranslateCacheKey(targetLanguage, trimmed);
    const cached = translatedValueCache.get(cacheKey);
    ...
    const translatedText = translated.translatedText || value;
    translatedValueCache.set(cacheKey, translatedText);
    return translatedText;
  }

  if (Array.isArray(value)) {
    return Promise.all(value.map((entry) => translateValue(entry, targetLanguage)));
  }

  if (value && typeof value === "object") {
    const entries = await Promise.all(
      Object.entries(value).map(async ([key, child]) => [key, await translateValue(child, targetLanguage)])
    );
...
function GuideCard({ topic, onClose, t }) {
  return (
    <div style={{
      ...
      <div style={{
          ...
          <div style={{ display: "flex", alignItems: "flex-start", gap: 12 }}>
            <div style={{ flex: 1 }}>
              <p style={{ color: "rgba(255,255,255,0.7)", fontSize: 12, margin: "0 0 4px", letterSpacing: "0.04em", textTransform: "uppercase" }}>
                {t("category.guideLabel", { defaultValue: "Guide" })}
              </p>
              ...
              <p style={{ color: "rgba(255,255,255,0.65)", fontSize: 12, margin: "6px 0 0" }}>
                {t("category.readTime", { defaultValue: "{{time}} read", time: topic.readTime })}
              </p>
            </div>
        ...
        {topic.tip && (
          <div style={{
            ...
            <div>
              <p style={{ fontSize: 11, fontWeight: 700, color: TEAL.mid, margin: "0 0 3px", textTransform: "uppercase", letterSpacing: "0.05em" }}>
                {t("category.proTip", { defaultValue: "Pro tip" })}
              </p>
...
export default function CategoryScreen() {
  ...
  const [translatedTopics, setTranslatedTopics] = useState(guideCategory?.topics || []);
  const [translatedSelectedTopic, setTranslatedSelectedTopic] = useState(null);
  const [isTranslatingTopic, setIsTranslatingTopic] = useState(false);
  const [isTranslatingList, setIsTranslatingList] = useState(false);

  const targetLanguage = (user?.language_code || i18n.resolvedLanguage || i18n.language || "en").split("-")[0];
  const categoryTitle = t(`home.categoryLabel.${slug}`, { defaultValue: formatSlug(slug) });

  useEffect(() => {
    let cancelled = false;
    if (!guideCategory?.topics?.length) {
      setTranslatedTopics([]);
      return undefined;
    }
    if (!targetLanguage || targetLanguage === "en") {
      setTranslatedTopics(guideCategory.topics);
      return undefined;
    }

    setIsTranslatingList(true);
    translateValue(guideCategory.topics, targetLanguage)
      .then((value) => {
        if (!cancelled) setTranslatedTopics(value);
      })
      .finally(() => {
        if (!cancelled) setIsTranslatingList(false);
      });

    return () => {
      cancelled = true;
    };
  }, [guideCategory, targetLanguage]);

  useEffect(() => {
    let cancelled = false;
    if (!selectedTopic) {
      setTranslatedSelectedTopic(null);
      setIsTranslatingTopic(false);
      return undefined;
    }

    if (!targetLanguage || targetLanguage === "en") {
      setTranslatedSelectedTopic(selectedTopic);
      setIsTranslatingTopic(false);
      return undefined;
    }

    setIsTranslatingTopic(true);
    translateValue(selectedTopic, targetLanguage)
      .then((value) => {
        if (!cancelled) setTranslatedSelectedTopic(value);
      })
      .finally(() => {
        if (!cancelled) setIsTranslatingTopic(false);
      });
  ...
  return (
    <main style={{ background: "#f7f8f6", minHeight: "100vh", paddingBottom: 80 }}>
      ...
      <div style={{ padding: "16px 16px 0" }}>
        {guideCategory ? (
          translatedTopics.map((topic, i) => (
            <button
              key={topic.id}
              onClick={() => setSelectedTopic(guideCategory.topics[i] || topic)}
              style={{
        ...
        {isTranslatingList ? (
          <p style={{ color: "#666", fontSize: 13, marginTop: 8 }}>
            {t("category.translatingList", { defaultValue: "Translating topics..." })}
          </p>
        ) : null}
      </div>
      ...
      {selectedTopic && (
        <GuideCard
          topic={translatedSelectedTopic || selectedTopic}
